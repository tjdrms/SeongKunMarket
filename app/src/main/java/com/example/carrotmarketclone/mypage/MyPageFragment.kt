package com.example.carrotmarketclone.mypage
...
import com.example.carrotmarketclone.databinding.FragmentMypageBinding
        ...
        fragmentMypageBinding.signUpButton.setOnClickListener {
            binding?.let { binding ->
                ...
                auth.createUserWithEmailAndPassword(email, password)
                    .addOnCompleteListener(requireActivity()) { task ->
                        if (task.isSuccessful) {
                            Toast.makeText(context, "회원가입에 성공했습니다! 로그인 버튼을 늘러주세요.", Toast.LENGTH_SHORT).show()
                        } else {
